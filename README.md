...
This fork always reports eligible requests regardless of cookie values. Incoming query parameters are deliberately excluded from recorded URLs, while the original request and query remain unchanged for downstream handlers.
...
- **static_metadata** is configuration-controlled information which will be added to the recorded URL as a query string. Incoming request query parameters are never included.
